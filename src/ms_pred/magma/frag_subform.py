...
class FragmentEngine(object):
    ...
    def __init__(
        ...
        self.atom_hs = np.array([i.GetNumImplicitHs() + i.GetNumExplicitHs()
                                 for i in self.mol.GetAtoms()])
        self.total_hs = self.atom_hs.sum()
        self.atom_weights = np.array([common.ELEMENT_TO_MASS[i]
                                      for i in self.atom_symbols])
        self.atom_weights_h = (self.atom_hs * common.ELEMENT_TO_MASS['H'] +
                               self.atom_weights)
        ...
        self.bonded_atoms_np = np.zeros((self.natoms, MAX_ATOM_BONDS),
                                        dtype=int)
        self.bonded_types_np = np.zeros((self.natoms, MAX_ATOM_BONDS),
                                        dtype=int)
        ...
        for bond in self.mol.GetBonds():
            ...
            bondbits = 1 << a1 | 1 << a2
            bondscore = (TYPEW[bond.GetBondType()] *
                         HETEROW[self.atom_symbols[a1] != "C" or
                                 self.atom_symbols[a2] != "C"])
    ...
    def atom_pass_stats(self, frag: int, depth: int = None):
        ...
        return {"form": form, "base_mass": float(fragment_mass),
                "max_remove_hs": max_remove, "max_add_hs": max_add,}
    ...
    def wl_hash(self, template_fragment: int,) -> int:
        ...
        cur_hashes = [f"{i}_{j}_{len(k)}" for i,j,k in zip(self.atom_symbols,
                                                            self.atom_hs,
                                                            self.bonded_atoms)]

        cur_hashes = [f"{i}" for i,j,k in zip(self.atom_symbols, self.atom_hs,
                                              self.bonded_atoms)]
    ...
    def get_frag_forms(self):
        ...
        for k, v in self.frag_to_entry.items():
            ...
            for num_shift, shift_ind, shift_mass in zip(self.shift_buckets,
                                                        self.shift_bucket_inds,
                                                        self.shift_bucket_masses):
                if (num_shift >= -max_remove) and (num_shift <= max_add):
                    new_form_vec = base_form_vec + num_shift * common.ELEMENT_VECTORS[common.element_to_ind['H']] 
                    str_code = str(new_form_vec)
                    if str_code in form_set: continue
    ...
    def generate_fragments(self):
        ...
        root.update(self.atom_pass_stats(frag,
                                         depth=0))
                ...
                for atom in range(self.natoms):
                    ...
                    for frag in extended_fragments:
                        ...
                        if old_entry is None:
                            ...
                            new_entry
                            new_entry.update(self.atom_pass_stats(frag,
                                                                  depth=max_broken))
    ...
    def remove_atom(self, fragment: int, atom: int) -> List[dict]:
        ...
        if len(list_ext_atoms) == 1:
            ...
            extended_fragments.append({"new_frag": template_fragment,
            ...
            for a in list_ext_atoms:
                ...
                if not is_ring:
                    ...
                    extended_fragments.append({'new_frag': new_fragment,
                                               'new_hash': new_frag_hash,
                                               'removed_atom': atom,
                                               })
