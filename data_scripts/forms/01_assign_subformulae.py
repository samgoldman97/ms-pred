...
def get_args():
    ...
    parser.add_argument("--data-dir",
                        default="data/spec_datasets/gnps2015_debug",
                        help="Data debug")
    ...
    parser.add_argument("--use-magma", action="store_true", default=False,
                        help='If true, do not subset formula.')
    parser.add_argument("--mass-diff-type", default='ppm',
                        type=str,
                        help='Type of mass difference - absolute differece (abs) or relative difference (ppm).')
    parser.add_argument("--mass-diff-thresh", action="store", default=10,
                        type=float,
                        help='Threshold of mass difference.')
    parser.add_argument("--inten-thresh", action="store", default=0.001,
                        type=float,
                        help='Threshold of MS2 subpeak intensity (normalized to 1).')
    parser.add_argument("--max-formulae", action="store", default=50,
                        type=int,
                        help="Max number of peaks to keep")
    return parser.parse_args()


def get_output_dict(spec_name: str, spec: np.ndarray, form: str, mass_diff_type: str,
                    mass_diff_thresh: float, inten_thresh: float, adduct_type: str,
                    max_formulae: int = 100, use_all=False,
                    smiles: str = "", use_magma: bool = False,
                    ) -> dict:
    """get_output_dict.
    ...
    if use_magma:
        ...
        try:
            ...
            return {'cand_form': form, 'spec_name': spec_name,
                    'cand_ion': adduct_type, 'output_tbl': None}
    ...
    formulas = np.array([common.vec_to_formula(j)
    ...
    for idx in range(len(formulas)):
        ...
        if formula not in formula_idx_dict:
            ...
            spec_intens[merge_idx] = max(spec_intens[idx],
                                         spec_intens[merge_idx])
...
def main():
    ...
    input_specs = common.chunked_parallel(spec_fn_lst,
                                          proc_spec_full,
                                          chunks=100
                                          )
    ...
    for _, row in labels_df.iterrows():
        spec = row['spec']
        new_entry = {"spec": input_specs_dict[spec],
                     "form": row['formula'],
    ...
    if debug:
        ...
        output_dict_lst = common.chunked_parallel(export_dicts,
                                                  export_wrapper,
                                                  chunks=100
                                                  )
