...
def get_args():
    ...
    parser.add_argument(
        "--min-inten", type=float, default=0,
    ...
    parser.add_argument(
        "--num-bins", type=int, default=3000,
        help="Num bins"
    )
    parser.add_argument(
...
def bin_dag(dag: dict, max_peaks: int, upper_limit: int, num_bins: int,
...
def bin_dag_file(dag_file: dict, max_peaks: int, upper_limit: int, num_bins: int,
                 min_inten: float):
    """bin_dag_file.
...
def main():
    ...
    read_dag_file = partial(bin_dag_file, max_peaks=max_peaks,
                            upper_limit=upper_limit, num_bins=num_bins,
                            min_inten=min_inten)
    ...
    if num_workers > 0:
        ...
        smis = common.chunked_parallel(inchis, common.smiles_from_inchi,
                                       max_cpu=num_workers)
    else:
